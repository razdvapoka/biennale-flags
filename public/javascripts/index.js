...
const toggleConept = () => {
  ...
  if (conceptBox.classList.contains('concept-box-open')) {
...
const updateAlert = () => {
  const now = new Date()
  if (isVotingOver || now > new Date('2021-12-02T00:00:01')) {
    colors.classList.add('inactive')
    isVotingOver = true
    alertText.innerText = 'voting is over'
    alertCountDown.innerText = ''
  } else {
    alertText.innerText = 'last voting day'
    alertCountDown.innerText =
      fixedLengthTimePart(now.getHours()) +
      ':' +
      fixedLengthTimePart(now.getMinutes()) +
      ':' +
      fixedLengthTimePart(now.getSeconds())
  }
}
...
const init = () => {
  ...
  alertIcon.addEventListener('click', () => {
    alert.classList.add('hidden-perm')
  })
